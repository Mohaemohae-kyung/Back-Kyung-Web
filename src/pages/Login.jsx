...
export default function Login({ setUser }) {
  ...
  return <AuthLayout title="로그인" desc="매칭온 계정으로 요청 내역과 채팅을 확인하세요."><form className="form" onSubmit={submit}><Input label="이메일" value={form.email} onChange={v => setForm({ ...form, email: v })} /><Input label="비밀번호" type="password" value={form.password} onChange={v => setForm({ ...form, password: v })} /><button className="btn btn-primary full">로그인</button><p className="muted center">이메일/비밀번호 찾기는 준비 중입니다.</p>{msg && <p className="message">{msg}</p>}</form></AuthLayout>;
}
