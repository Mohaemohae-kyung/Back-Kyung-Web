...
export default function Header({ user, logout }) {
  ...
  const nav = [['/', '홈'], ['/experts', '고수찾기'], ['/market', '마켓'], ['/requests', '요청관리'], ['/community', '커뮤니티'], ['/mypage', '마이페이지']];
  const close = () => setOpen(false);
  return <header className="topbar">
    <div className="container nav-wrap">
      <Link className="brand" to="/" onClick={close}>
        <span className="brand-icon"><HeartHandshake size={24} /></span>
        <div><b>매칭온</b><small>생활서비스 매칭 플랫폼</small></div>
      </Link>
      ...
      <div className="auth-area">
        {user ? <>
          <span className="hello">{user.nickname || user.name || '사용자'}님</span>
          <button className="btn btn-ghost" onClick={logout}><LogOut size={16} />로그아웃</button>
        </> : <>
